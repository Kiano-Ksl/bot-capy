...
/* KOTAK ARCADE: Mengisolasi game agar aman dari Light Mode WA */
#app{
  max-width:420px;
  margin:0 auto;
  background-color: #050b14;
  background-image:
    radial-gradient(circle at 50% 0%, #173872 0%, transparent 70%),
    linear-gradient(rgba(88,199,255,0.06) 1px, transparent 1px),
    linear-gradient(90deg, rgba(88,199,255,0.06) 1px, transparent 1px);
  background-size: 100% 100%, 20px 20px, 20px 20px;
  padding:16px;
  border-radius:24px;
  border:2px solid #173872;
  box-shadow:0 12px 30px rgba(0,0,0,0.7), inset 0 0 20px rgba(0,0,0,0.5);
  color:#eaf2ff;
}

.hdr{display:flex;flex-direction:column;justify-content:center;align-items:center;padding:0 2px 12px;gap:8px}
.tt{font:900 18px 'Arial Black';color:#58c7ff;text-shadow:0 0 12px #58c7ff66;letter-spacing:1px;text-align:center;line-height:1}
.tt small{display:block;font:700 7.5px Arial;letter-spacing:2px;color:#7a9cc8;text-shadow:none;margin-top:4px}
.hrs{display:flex;gap:6px;align-items:center;justify-content:center;width:100%}
.hr{background:rgba(0,0,0,.5);border:1px solid rgba(88,199,255,.3);border-radius:10px;padding:4px 8px;text-align:center;min-width:48px;flex:1;box-shadow:inset 0 0 8px rgba(0,0,0,.5)}
.hr i{display:block;font:700 7px Arial;font-style:normal;letter-spacing:1px;color:#7a9cc8}
.hr b{font:900 13px 'Arial Black';color:#ffd75e;font-variant-numeric:tabular-nums;text-shadow:0 0 6px rgba(255,215,94,.4)}
.mbtn{width:34px;height:34px;border:2px solid rgba(88,199,255,.3);border-radius:10px;background:rgba(0,0,0,.5);color:#fff;font-size:15px;cursor:pointer;touch-action:none;flex:none;box-shadow:inset 0 0 8px rgba(0,0,0,.5)}
.mbtn:active{filter:brightness(1.6);transform:scale(0.95)}
.gw{position:relative;border:3px solid rgba(88,199,255,.4);border-radius:16px;overflow:hidden;background:#000;box-shadow:0 0 20px rgba(88,199,255,.2)}
canvas{width:100%;display:block;touch-action:none}
.pads{display:grid;grid-template-columns:1fr 1.3fr;gap:12px;margin-top:12px}
.pd{height:54px;border:2px solid rgba(255,255,255,.2);border-radius:14px;font:900 15px 'Arial Black';color:#fff;cursor:pointer;touch-action:none;box-shadow:0 5px 0 rgba(0,0,0,.6), inset 0 2px 5px rgba(255,255,255,.2)}
.pd:active{transform:translateY(4px);box-shadow:none;filter:brightness(1.3)}
#boostB{background:linear-gradient(#ffd75e,#e09406 60%,#7a5205);color:#3a2805;text-shadow:0 1px 2px rgba(255,255,255,.4)}
#jumpB{background:linear-gradient(#58c7ff,#1f7fd6 60%,#0a3a6e);text-shadow:0 1px 2px rgba(0,0,0,.4)}
.credit{text-align:center;font:600 10px Arial;color:#647a9f;margin-top:14px;padding-bottom:2px}
#stopB{width:100%;height:40px;margin-top:12px;background:rgba(255,255,255,.08);font-size:11px;border:1px solid rgba(255,255,255,.2);border-radius:12px;color:#a0b8d8;font-weight:bold;box-shadow:0 3px 0 rgba(0,0,0,.4);cursor:pointer;}
#stopB:active{transform:translateY(3px);box-shadow:none;background:rgba(255,255,255,.15);}
</style>
<div id="app">
<div class="hdr">
  <div class="tt">🌀 SPEEDY DASH<small>EMERALD COAST RUN</small></div>
  <div class="hrs">
    <div class="hr"><i>RINGS</i><b id="rg">0</b></div>
...
<button class="pd" id="stopB">↻ RESTART / STOP GAME</button>
<div class="credit">Code by: Anita Putri Azzahra</div>
</div>
<script>
...
document.addEventListener('pointerdown',function(e){if(e.target.closest('.pads,.mbtn,#stopB'))return;if(e.clientX<innerWidth/2)doBoost();else jump()});
...
/* --- TOMBOL RESTART & ANTI-ZOMBIE AUDIO --- */
document.getElementById('stopB').addEventListener('pointerdown', function(e){
    e.preventDefault();
    state = 'ready'; 
    if(AC){try{AC.suspend();}catch(err){}} 
    reset(); 
});
...
x.font='600 9px Arial';x.fillStyle='#7a9cc8';x.fillText('laser merah kedip cepat = LOMPAT! · 50 ring = BOSS',W/2,198);
x.textAlign='left';
}
if(state==='dead'){
...
async function handleSonic(sock, msg, from) {
    try {
        await kirimForwardSigned(sock, from, DASH_HTML, '🌀 SPEEDY DASH v4');
    } catch (e) {
        console.error('[SONIC]', e?.message || e);
        await sock.sendMessage(from, { text: '❌ Gagal mengirim game Sonic: ' + (e?.message || e) }, { quoted: msg });
    }
}

module.exports = handleSonic;