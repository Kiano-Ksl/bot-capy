...
<div class="raceWrap">
  <div class="raceHeader">
    <div class="raceTitle">🏎️ TURBO <span>RACE</span></div>
    <div class="raceStats">
...
// MATIKAN AUDIO SAAT KELUAR DARI WA CHAT
document.addEventListener('visibilitychange', function() {
    if (document.hidden) AU.suspend();
});
window.addEventListener('blur', function() { AU.suspend(); });
window.addEventListener('pagehide', function() { AU.suspend(); });
...
function drawSpeedo(){
    ...
    for(var i=0;i<=8;i++){
        var a=a0+(a1-a0)*i/8;
        ctx.beginPath();
        ctx.moveTo(cx+Math.cos(a)*(R-9),cy+Math.sin(a)*(R-3));
...
async function handleBalap(sock, msg, from) {
  try {
    console.log(`[BALAP] Game dijalankan di chat: ${from}`);
    if (msg.key) {
    ...
    const data = Buffer.from(
      JSON.stringify({
        ...
        response_id: "balap-racing-" + randomUUID(),
        sections: [
          {
    ...
    await sock.sendMessage(from, { text: "❌ Gagal memuat game Balap: " + (err?.message || err) }, { quoted: msg });
  }
}

module.exports = handleBalap;