...
async function gemini(input = {}) {
    ...
    try {
        ...
        const pLoad = [null, JSON.stringify(requestBody)];
        ...
        const { data } = await axios.post(
            ...
            new URLSearchParams({ "f.req": JSON.stringify(pLoad) }).toString(),
            {
                headers: {
        ...
        const newSessionId = Buffer.from(JSON.stringify({ resumeArray: newResumeArray, cookie: cookie, instruction: savedInstruction })).toString("base64");
